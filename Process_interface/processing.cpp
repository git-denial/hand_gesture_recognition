...
{   
    faces.clear();
    cvtColor( Processedframe, frame_gray,  COLOR_BGR2GRAY );
...
bool Processing::detectHandFist()
{
    hands.clear();
    cvtColor( Processedframe, frame_gray, COLOR_BGR2GRAY );
    fist_cascade.detectMultiScale( frame_gray, hands, 1.1, 2,cv::CASCADE_DO_CANNY_PRUNING, Size(40,40) );
...
bool Processing::detectHandPalm()
{
    handsPalm.clear();
    cvtColor( Processedframe, frame_gray, COLOR_BGR2GRAY );
    hand_palm_cascade.detectMultiScale( frame_gray, handsPalm, 1.1, 2, cv::CASCADE_DO_CANNY_PRUNING, Size(40,40) );
...
bool Processing::detectFingerAndCentroidInOpenPalm(cv::Mat& frame)
{
    /**************    *********/
    // Find contours
    cv::findContours( mask.clone(), contours, hierarchy, cv::RETR_LIST, cv::CHAIN_APPROX_TC89_KCOS, Point(0, 0) );
    ...
    for( int i = 0; i < contours.size(); i++ )
    {
        ...
        for(size_t k = 0; k<hullsI[i].size(); k++){
            if(hullsI[i].size() > 0){
...
//    bool facePresent = detectFace();
    ...
    if(gesture != "none") cout<<gesture<<endl;
    cv::putText(drawing, gesture, cv::Point(30,30), FONT_HERSHEY_COMPLEX_SMALL, 0.8, Scalar(200,200,250), 1, cv::LINE_AA);

    /*********************              Display             *******************************/

    /** convert to RGB for display **/
    cv::cvtColor(Processedframe, Processedframe, COLOR_BGR2RGB );
